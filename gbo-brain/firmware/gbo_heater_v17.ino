...
uint32_t coldLimit(const Sensor &other) {
...
uint16_t coldLeftSec() {
  ...
  if (S3.st == SENS_COLD && S3.coldSince) {
    uint32_t lim = coldLimit(S4), el = now - S3.coldSince;
    best = min(best, (el >= lim) ? 0UL : (unsigned long)(lim - el));
  }
  if (S4.st == SENS_COLD && S4.coldSince) {
    uint32_t lim = coldLimit(S3), el = now - S4.coldSince;
    best = min(best, (el >= lim) ? 0UL : (unsigned long)(lim - el));
  }
...
void checkProtections() {
  ...
  if (state == ST_RUN && duty >= COLD_DUTY) {
    if (S3.st == SENS_COLD && S3.coldSince && now - S3.coldSince >= coldLimit(S4)) raiseFault(1);
    if (S4.st == SENS_COLD && S4.coldSince && now - S4.coldSince >= coldLimit(S3)) raiseFault(2);
    if (faultLatched) return;
  } else {
