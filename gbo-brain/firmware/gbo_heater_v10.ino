/* =====================================================================
   ...
   ---------------------------------------------------------------------
   v10, что изменено против v3 (1-3 из v4, 4 из v5, 6 из v6, 7 из v7, 8-9 из v8, 10 из v9):
...
/* Своя точка поднимается всегда, вместе с домашней сетью. Так плата
   доступна по 192.168.4.1 при любом раскладе: запоротый пароль, переезд
   роутера или смена канала не оставляют без связи. */
void startWifi() {
  ...
  WiFi.setSleep(false);          // со сном в этом режиме сервер отвечает рывками
