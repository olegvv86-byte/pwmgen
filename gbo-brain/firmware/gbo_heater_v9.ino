/* =====================================================================
   ...
   ---------------------------------------------------------------------
   v9, что изменено против v3 (1-3 из v4, 4 из v5, 6 из v6, 7 из v7, 8-9 из v8):
  ...
  10. Домашняя сеть задаётся через страницу /wifi и живёт в NVS, в исходник
      её не пишем — файл уходит в репозиторий. Не подключилась за 10 секунд
      — поднимается своя точка, без связи не останемся. Плюс имя gbo.local,
      чтобы не искать адрес: http://gbo.local/update.
...
<label>Имя сети</label><input name="s" maxlength="32" value="%SSID%">
...
/* Поднять сеть: сперва домашняя, при неудаче своя точка. */
void startWifi() {
